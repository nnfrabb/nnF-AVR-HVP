// AVR-HVP (AVR Hight Voltage Programmer) by nnFrabb
// For ATMEGA8/8L/88 // Osc 8MHz Internal // Baudrate 250000
...
int main() {
	...
	for(;;) {
		...
		if((arg0=="+FCTRST")&&(prgMode!=0)) {
		...
		}  else if((arg0=="+WRFLSH")&&(prgMode!=0)) {//+wrflsh  arg0 = inputCMD.substring(2,9);
			uint32_t hexsize = inputCMD.substring(9,14).toInt();
			uint32_t pagesize = inputCMD.substring(14,17).toInt();
			uint32_t addr = inputCMD.substring(17,22).toInt();
			uint32_t pagemask = ~(pagesize - 1);
			uint32_t oldPage = addr & pagemask;
			bool wrcmp = false;
			sinfo = false;
			...
			if(prgMode == 1 ) {
				...
				Serial.println(hexsize);

				for(uint32_t i=0;i<hexsize;i+=2) {
					String hdata = "";
					hdata.reserve(64);
					uint8_t hexd[16] = {0};
					bool lhd = true;
					...
					uint32_t nbyte = hdata.substring(0,2).toInt()-1;
					uint32_t haddr = hdata.substring(2,7).toInt();
					uint8_t m = 7;

					for(uint32_t k=0;k<=nbyte;k++) {
						hexd[k] = hdata.substring(m,m+3).toInt();
						m += 3;
						if(hexd[k]<0x10) Serial.print(0);
						Serial.print(hexd[k], HEX);
					}
					Serial.println(hexsize);

/*					for(uint32_t j=0;j<=nbyte;j+=2) {
						lhd = true;
						uint32_t thisPage = (haddr + i) & pagemask;
						if (thisPage != oldPage) {
							hvsp_commitPage(oldPage,pagesize);
							oldPage = thisPage;
						}

						hvsp_writeFlash(haddr+j,hdata[j]);
						hvsp_writeFlash(haddr+j+1,hdata[j+1]);
						i += 2;
					}
*/				}
//				hvsp_commitPage(oldPage,pagesize);
